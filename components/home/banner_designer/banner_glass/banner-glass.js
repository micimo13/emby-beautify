/* Vanvy Emby Kit V2 · GLASS 玻璃拟态 轮播组件 (由效果图演进) */
(function () {
  'use strict';
  if (window.VanvyGlass) return;
  window.VanvyGlass = {};

  window.VanvyCarouselCore.register('banner_glass', {
    query: {
      ImageTypes: 'Backdrop',
    ...
    render: function (items, root, core) {
      // 容器结构
      root.innerHTML = '<div class="glass">' + '<div class="vbd-bd"></div><div class="vbd-scrim"></div>' +
        '<div class="glass-slide">' +
        '<div class="glass-poster" data-poster><div data-overlay></div></div>' +
        '  <div class="glass-info">' +
        '    <div class="glass-title"></div>' +
        '    <div class="glass-sub"></div>' +
        '    <div class="glass-meta"><span class="glass-rating">★ </span><span class="glass-year"></span><span class="glass-genres"></span></div>' +
        '    <div class="glass-desc"></div>' +
        '    <div class="glass-btns">' +
        '      <button class="glass-btn glass-play" data-action="play">▶ 播放</button>' +
        '      <button class="glass-btn glass-detail" data-action="detail">ⓘ 详情</button>' +
        '    </div>' +
        '  </div>' +
        '</div>' +
        '<div class="glass-thumbs"></div>' +
      '</div>';

      var idx = 0;
      var titleEl = root.querySelector('.glass-title');
      var subEl = root.querySelector('.glass-sub');
      var descEl = root.querySelector('.glass-desc');
      var ratingEl = root.querySelector('.glass-rating');
      var yearEl = root.querySelector('.glass-year');
      var genresEl = root.querySelector('.glass-genres');
      var posterEl = root.querySelector('[data-poster]');
      var bdEl = root.querySelector('.vbd-bd');
      var thumbsEl = root.querySelector('.glass-thumbs');
      ...
      function renderThumbs() {
        ...
        items.forEach(function (it, i) {
          ...
          t.className = 'glass-thumb' + (i === idx ? ' active' : '');
  ...
  window.VanvyGlass.start = function () {
    window.VanvyCarouselCore.start('banner_glass');
  };
})();
