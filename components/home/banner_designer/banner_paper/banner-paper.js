/* Vanvy Emby Kit V2 · PAPER 纸艺 轮播组件 (由效果图演进) */
(function () {
  'use strict';
  if (window.VanvyPaper) return;
  window.VanvyPaper = {};

  window.VanvyCarouselCore.register('banner_paper', {
    query: {
      ImageTypes: 'Backdrop',
    ...
    render: function (items, root, core) {
      // 容器结构
      root.innerHTML = '<div class="paper">' + '<div class="vbd-bd"></div><div class="vbd-scrim"></div>' +
        '<div class="paper-slide">' +
        '<div class="paper-poster" data-poster><div data-overlay></div></div>' +
        '  <div class="paper-info">' +
        '    <div class="paper-title"></div>' +
        '    <div class="paper-sub"></div>' +
        '    <div class="paper-meta"><span class="paper-rating">★ </span><span class="paper-year"></span><span class="paper-genres"></span></div>' +
        '    <div class="paper-desc"></div>' +
        '    <div class="paper-btns">' +
        '      <button class="paper-btn paper-play" data-action="play">▶ 播放</button>' +
        '      <button class="paper-btn paper-detail" data-action="detail">ⓘ 详情</button>' +
        '    </div>' +
        '  </div>' +
        '</div>' +
        '<div class="paper-thumbs"></div>' +
      '</div>';

      var idx = 0;
      var titleEl = root.querySelector('.paper-title');
      var subEl = root.querySelector('.paper-sub');
      var descEl = root.querySelector('.paper-desc');
      var ratingEl = root.querySelector('.paper-rating');
      var yearEl = root.querySelector('.paper-year');
      var genresEl = root.querySelector('.paper-genres');
      var posterEl = root.querySelector('[data-poster]');
      var bdEl = root.querySelector('.vbd-bd');
      var thumbsEl = root.querySelector('.paper-thumbs');
      ...
      function renderThumbs() {
        ...
        items.forEach(function (it, i) {
          ...
          t.className = 'paper-thumb' + (i === idx ? ' active' : '');
  ...
  window.VanvyPaper.start = function () {
    window.VanvyCarouselCore.start('banner_paper');
  };
})();
