...
export TMPDIR="$TMPD"
  ...
  local hook="$TMPD/vanvy-persist-hook.sh"
  cat > "$hook" << EOF
#!/bin/sh
...
EOF
  docker cp "$hook" "$CONTAINER:$TMPD/vanvy-persist-hook.sh" 2>/dev/null
  # 插到 exit 0 前
  docker exec "$CONTAINER" sh -c "
    EXT='$ext_file'
    if grep -q '^exit 0' \"\$EXT\"; then
      awk 'BEGIN{while((getline l < \"$TMPD/vanvy-persist-hook.sh\")>0) h=h l \"\\n\"} /^exit 0/ && !d {printf \"%s\", h; d=1} {print}' \"\$EXT\" > \"\$EXT.new\" && mv \"\$EXT.new\" \"\$EXT\"
    else
      cat $TMPD/vanvy-persist-hook.sh >> \"\$EXT\"
    fi
    rm -f $TMPD/vanvy-persist-hook.sh
    echo '  ✓ 持久化钩子已写入'
  " 2>&1 | sed 's/^/    /'
  rm -f "$hook"
