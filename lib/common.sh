...
export TMPDIR="$TMPD"
  ...
  local filtered="$TMPD/vanvy-filtered.html"
  : > "$filtered"
  ...
  # 上传过滤后的注入文件
  docker cp "$filtered" "$CONTAINER:$TMPD/vanvy-inject-final.html" 2>/dev/null
  docker exec -i "$CONTAINER" sh -c "
    set -e
    INDEX='$index_file'
    if [ ! -s $TMPD/vanvy-inject-final.html ]; then
      echo '[错误] 注入文件为空, 已中止'
      exit 1
    fi
    mkdir -p /config/backups/emby-beautify
    ...
    awk -v anchor=\"\$ANCHOR\" 'FNR==NR { lines[NR]=\$0; n=NR; next } index(\$0, anchor) > 0 && !injected { for (i=1; i<=n; i++) print lines[i]; injected=1 } { print }' \\
      $TMPD/vanvy-inject-final.html \"\$INDEX\" > \"\$INDEX.new\"
    rm -f $TMPD/vanvy-inject-final.html
    if [ -s \"\$INDEX.new\" ]; then
      mv \"\$INDEX.new\" \"\$INDEX\"
