(function () {
    "use strict";
    // ── 倍速记忆 (吸收 RememberPlaybackSpeed: 保存+恢复+防重置) ──
    // 存储 key 用 vanvy_ 前缀, 避免与其他插件/群晖脚本撞 key
    var SPEED_KEY = "vanvy_playback_speed";
    var isTheater = !!(window.require && window.process && window.process.versions && window.process.versions.electron);
    var savedSpeed = null;
    try { savedSpeed = parseFloat(localStorage.getItem(SPEED_KEY)) || null; } catch (e) { savedSpeed = null; }
    if (savedSpeed && Math.abs(savedSpeed - 1) < 0.01) savedSpeed = null; // 1x 不记忆
    function saveSpeed(rate) {
        if (!rate || Math.abs(rate - 1) < 0.01) return;
        try { localStorage.setItem(SPEED_KEY, rate.toString()); } catch (e) { /* ignore */ }
    }
    function restoreSpeed() {
        if (!savedSpeed || !view || !view.currentPlayer) return;
        try {
            var cur = view.currentPlayer.getPlaybackRate ? view.currentPlayer.getPlaybackRate() : null;
            if (cur !== savedSpeed && Math.abs((cur || 1) - 1) < 0.01) {
                view.currentPlayer.setPlaybackRate(savedSpeed);
            }
        } catch (e) { /* ignore */ }
    }
    ...
    document.addEventListener("viewbeforeshow", function (e) {
        if (e.detail.type === "video-osd") {
            ...
            // 倍速记忆: 进入播放页恢复上次倍速 (延迟等播放器就绪)
            if (savedSpeed) {
                setTimeout(function () {
                    try {
                        restoreSpeed();
                        // 防重置保护: 前 5 秒检查, 防止被 Emby 重置为 1x
                        var count = 0;
                        var protect = setInterval(function () {
                            try {
                                if (view && view.currentPlayer && Math.abs(view.currentPlayer.getPlaybackRate() - 1) < 0.01) {
                                    view.currentPlayer.setPlaybackRate(savedSpeed);
                                }
                            } catch (err) { /* ignore */ }
                            if (++count >= 5) clearInterval(protect);
                        }, 1000);
                    } catch (err) { /* ignore */ }
                }, 800);
            }
        ...
        if (my_touches_start == null && !view.currentVisibleMenu && (!view.currentLockState || view.currentLockState === 0)) {
            ...
            my_touches_time = setTimeout(() => {
                if (
                    ...
                    saveSpeed(my_touches_rate * 2); // 倍速记忆: 触摸双击也记录
                    view.boundHideOsd();
                    require(["toast"], function (toast) {
