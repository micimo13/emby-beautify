...
class Home {
	static start() {
		...
		setInterval(() => {
			if (window.location.href.indexOf("!/home") != -1) {
				...
				if (document.documentElement.getAttribute('data-heicha-failed')) return;
	...
	static async init() {
		// Vanvy 策展规则: 有规则则覆盖默认查询 (无规则/失败回退默认, 向后兼容)
	...
	static async applyCarouselRules() {
		try {
			if (window.VanvyCarouselRules && window.VanvyCarouselRules.loaded) {
				...
				if (q) { this.itemQuery = q; console.log('[Emby-Fluent] 已应用策展规则查询'); }
			}
		} catch (e) {
			console.warn('[Emby-Fluent] 策展规则应用失败, 使用默认查询', e);
		}
	}
	...
	static getImageUrl(itemId, options) {
		const safeId = typeof itemId === "string" ? JSON.stringify(itemId) : itemId;
		return this.injectCall("getImageUrl", safeId + ", " + JSON.stringify(options));
	}
	...
	static async initBanner() {
		...
		if (window.VanvyCarouselRules && window.VanvyCarouselRules.loaded && data.Items) {
		...
		if (remainingSlides === 0) {
			...
			// 失败保护: 立即清理所有残留, 标记失败防止 watch 循环重试
			$(".heicha-banner").remove();
			$(".heicha-loading").remove();
			$(".heicha-banner-library").remove();
...
if ("BroadcastChannel" in window || "postMessage" in window) {
	const _meta = $("meta[name=application-name]").attr("content") || "";
	const _isEmby = /emby/i.test(_meta) || $(".accent-emby").length > 0 || !!document.querySelector(".skinBody, .mainAnimatedPages");
	if (_isEmby) {
		Home.start();
	}
}
