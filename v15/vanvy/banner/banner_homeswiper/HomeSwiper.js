...
.mdl-spinner {
	/* Vanvy 修复: 移除 zoom:0.5 (border 圆弧非整数像素渲染导致转圈裂开) */
}
		...
		document.addEventListener("viewbeforeshow", async function (e) {
			...
			if (e.detail.type === "home") {
				if (!e.detail.isRestored) {
					!document.getElementById("SwiperCss") && HomeSwiperUtils.loadExtastyle(this.SwiperCss, 'SwiperCss');
					!document.getElementById("customCss") && HomeSwiperUtils.loadExtastyle(this.customCss, 'customCss');
					...
					!localStorage.getItem('CACHE|Movies-Date') && !this.loadFlag && window.location.hash.endsWith("/home") && this.initLoading();
